...
static bool server_alive(ckpool_t *ckp, server_instance_t *si, bool pinging)
{
	...
	/* DigiByte's getblocktemplate always includes pow_algo. Refuse a node that
	...
	if (!ckp->node && !validate_address(cs, ckp->btcaddress, &ckp->script, &ckp->segwit)) {
		LOGWARNING("Invalid dgbaddress: %s !", ckp->btcaddress);
		goto out;
	}
...
static server_instance_t *live_server(ckpool_t *ckp, gdata_t *gdata)
{
	...
	LOGDEBUG("Attempting to connect to digibyted");
retry:
	...
	LOGWARNING("CRITICAL: No digibyteds active!");
	sleep(5);
	goto retry;
living:
...
reconnect:
	...
	cs = &si->cs;
	if (!old_si)
		LOGWARNING("Connected to digibyted: %s:%s", cs->url, cs->port);
	else if (si != old_si)
		LOGWARNING("Failed over to digibyted: %s:%s", cs->url, cs->port);
...
retry:
	...
	if (unlikely(!si->alive)) {
		LOGWARNING("%s:%s Digibyted socket invalidated, will attempt failover", cs->url, cs->port);
		goto reconnect;
	}
